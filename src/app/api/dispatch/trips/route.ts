...
export async function POST(request: Request) {
  try {
    ...
    const derivedStatus = deriveTripStatus({ startDate: start_date || null, endDate: end_date || null, requestedStatus: status || null });
...
export async function GET(request: Request) {
  try {
    ...
    `, scope.params);
    return NextResponse.json(trips.map((trip: any) => ({
      ...trip,
